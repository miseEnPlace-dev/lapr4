...
| 8    | `GET_BOARDS`            | Used to get boards which the authenticated user has permission to access.                                                                                                                                                                                                                              |
| 9    | `GET_OWN_BOARDS`        | Used to get boards which the authenticated user owns.                                                                                                                                                                                                                                                  |
| 10   | `GET_WRITABLE_BOARDS`   | Used to get boards which the authenticated user has write permission.                                                                                                                                                                                                                                  |
| 11   | `GET_BOARD`             | Used to get a board by specifying the desired id in the payload. The authenticated user have at least read permission to the board.                                                                                                                                                                    |
| 12   | `GET_USER_PERMISSIONS`  | Used to retrieve the permissions a specific user has to a certain board. The authenticated user must have at least read permission to the board.                                                                                                                                                       |
| 13   | `SHARE_BOARD`           | Used to modify another user's permission to a board. The authenticated user must be owner of the board.                                                                                                                                                                                                |
| 14   | `ARCHIVE_BOARD`         | Used to archive a user's board. The authenticated user must be owner of the board.                                                                                                                                                                                                                     |
| 15   | `GET_POSTITS_BOARD`     | Used to get the post-its of a board. The authenticated user must have at least read permission to the board.                                                                                                                                                                                           |
| 16   | `GET_OWN_POSTITS_BOARD` | Used to get the authenticated user's post-its of a board. The authenticated user must have at least read permission to the board.                                                                                                                                                                      |
| 17   | `GET_BOARD_HISTORY`     | Used to get the history of a specific board. The authenticated user must have at least read permission to the board.                                                                                                                                                                                   |
| 18   | `CREATE_POSTIT`         | Used to create a post-it in a specific board. The authenticated user must have write permission to the board.                                                                                                                                                                                          |
| 19   | `CHANGE_POSTIT`         | Used to edit a post-it. The authenticated user must be owner of the post-it.                                                                                                                                                                                                                           |
| 20   | `UNDO_POSTIT`           | Used to undo the last change in a post-it. The authenticated user must be owner of the post-it and have write permission to the board.                                                                                                                                                                 |
| 21   | `DELETE_POSTIT`         | Used to delete a post-it. The authenticated user must be owner of the post-it and have write permission to the board.                                                                                                                                                                                  |
| 22   | `IS_CELL_AVAILABLE`     | Used to check if a certain cell does not have a post-it in it. The authenticated user must have at least read permission to the board.                                                                                                                                                                 |
| 23   | `GET_ONLINE_COUNT`      | Used to get the number of clients currently connected to the server.                                                                                                                                                                                                                                   |

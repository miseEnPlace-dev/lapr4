 ...
 * instance, as such it
 * violates the rule that one controller should only create/change one aggregate
 * instance.
 * <p>
 * <p/>
 * We provide two implementations of this controller for comparison of different
 * approaches. The
 * (traditional) approach {@link AcceptRefuseSignupRequestControllerTxImpl}
 * creates a wrapping
 * transaction and performs the needed steps inside that transaction. It makes
 * it simple to develop
 * and understand but hardcodes the process and fails the Single Responsibility
 * Principle (SRP) as
 * now this class will have more than one reason to change.
 * <p>
 * <p/>
 * The other approach uses events
 * {@link AcceptRefuseSignupRequestControllerEventfulImpl} and
 * decouples the different steps of the process. This makes the system more
 * flexible if there is the
 * need to change the actual process, enforces the SRP, but increases the
 * complexity, e.g., there is
 * the need to have an event bus, there is no global status, if something fails
